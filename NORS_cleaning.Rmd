...
save this and then next chunk will read it in so i don't need to run this shit every time
```{r}

write.csv(dropped, "dropped.csv")
...
years_test = refined |> group_by(Year) |> 
  summarize(n = n())

years_test |> ggplot(aes(Year))+
